...
public class BankingSystem extends JFrame {
    ...
    public BankingSystem() {
        bank = new Bank("NovaBank", "Nethmi Umesha");
    ...
    private void doApplyLoan() {
        if (lnAmount.getText().trim().isEmpty()) {
            showMsg(lnAlert, "Please enter a loan amount.", UITheme.RED); return;
        }
        if (lnTerm.getText().trim().isEmpty()) {
            showMsg(lnAlert, "Please enter the loan term (e.g. 24 months).", UITheme.RED); return;
        }
        if (accIdFromCombo(lnAccount).isEmpty()) {
            showMsg(lnAlert, "Please select a deposit account.", UITheme.RED); return;
        }
        try {
            ...
            int term;
            try {
                term = Integer.parseInt(lnTerm.getText().trim());
            } catch (NumberFormatException e) {
                showMsg(lnAlert, "Term must be a whole number (e.g. 24).", UITheme.RED); return;
            }
            if (term <= 0)  { showMsg(lnAlert, "Term must be at least 1 month.", UITheme.RED); return; }
            if (term > 360) { showMsg(lnAlert, "Term cannot exceed 360 months.", UITheme.RED); return; }
            ...
            showMsg(lnAlert, ex.getMessage(), UITheme.RED);
        }
    }
